    ...
    <div className="fixed inset-0 z-modal flex items-center justify-center bg-black/35 p-5">
      ...
      >
        <header className="flex h-14 items-center justify-between border-b border-border px-5">
          <div>
            <h2 id="todo-workflow-dialog-title" className="text-lg font-semibold text-text-primary">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {initialConfig.statuses.map(status => (
              <span
                key={status.key}
                className="rounded-md border border-border bg-muted/40 px-2 py-1 text-xs text-text-secondary"
              >
                {status.name}
              </span>
            ))}
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {TODO_WORKFLOW_TEMPLATES.map(template => (
              <button
                ...
                className="h-7 rounded-md border border-border px-2 text-xs text-text-secondary hover:border-primary/40 hover:text-primary"
              >
                {template.name}
              </button>
            ))}
              ...
              <div key={workType.key} className="px-3 py-2">
                <div className="flex h-8 items-center gap-2">
                  <input
                    ...
                    className="h-7 min-w-0 flex-1 rounded-md border border-transparent bg-transparent px-1 text-xs text-text-primary outline-none hover:border-border focus:border-primary"
                  />
                  <span className="font-mono text-xs text-text-muted">{workType.key}</span>
                  <button
                    type="button"
          ...
          <div className="mt-3 flex gap-2">
            <input
              ...
              className="h-8 min-w-0 flex-1 rounded-md border border-border bg-background px-3 text-xs outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
            <Button
              type="button"
              variant="secondary"
